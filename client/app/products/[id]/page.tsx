...
import { productAPI, voteAPI, Product, Vote, SentimentAnalysis } from '@/lib/api';
    ...
    <div className="container mx-auto px-4 py-12">
        ...
        <div>
          ...
          <div className="flex items-center gap-6 mb-6">
            <div>
              ...
              <p className="text-sm text-gray-600">{product.totalVotes} votes</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="card">
          ...
          <VoteForm productId={product._id} onVoteSubmitted={handleVoteSubmitted} />
        </div>
          ...
          <div className="space-y-4 max-h-96 overflow-y-auto">
            {votes.length === 0 ? (
              ...
              votes.slice(0, 10).map(vote => (
                <div key={vote._id} className="border-3 border-pink-500 rounded-tl-4xl rounded-br-4xl rounded-tr-4xl p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-yellow-500">{'★'.repeat(vote.rating)}</span>
                    <span className={`px-2 py-1 rounded text-xs font-medium ${
                      vote.sentiment === 'positive' ? 'bg-green-100 text-green-800' :
                      vote.sentiment === 'negative' ? 'bg-red-100 text-red-800' :
